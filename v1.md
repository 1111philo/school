...
* Log in using platform credentials OR create a new user account.  
* Required: email/username or password.
...
### **B) lesson\_planner (lesson plan agent)**
...
If you want, I can also provide a **fully worked example output** for one objective from one of your preloaded courses (so you can test the agent end-to-end).
...
* **Modern, simple, clean** visual style with minimal visual noise.  
* Prioritize **readability**, **clear hierarchy**, and **low cognitive load**.  
* Consistent spacing, typography scale, and component styling across the app.  
* **Accessible by default** (color contrast, focus states, keyboard navigation).
...
* Theme switching behavior:  
  ...
  * Allow user override in settings (persist choice per user/device).  
* All UI components must render correctly in both themes:  
  * text, icons, borders, cards, nav, modals, forms, progress indicators, badges.

### **Layout \+ UI patterns**

* **Left navigation** on course page:  
  * clear active state  
  * locked/unlocked states visually distinct  
  * compact but readable lesson/activity labels  
* Main content area:  
  * card-based or sectioned layout  
  * generous padding and line length optimized for reading  
* Feedback screens:  
  * visually separate **score**, **rationale**, **strengths**, **improvements**, **next steps**  
* Agent Log viewer:  
  * developer-tool aesthetic (structured, scannable)  
  * prompt/output sections collapsible  
  * monospace for prompts/JSON where appropriate

### **Typography**

* Use a clean sans-serif UI font (system font acceptable).  
* Support:  
  * adjustable base font size  
  * comfortable line height for long-form lessons  
* Clear heading levels for lesson content (H1–H3 equivalents).

### **Color \+ components**

* Use a restrained palette; color is primarily for:  
  * status (success/warning/error)  
  * progress  
  * focus/selection states  
* Provide consistent component states:  
  * hover / active / disabled  
  * focus ring visible in both themes  
  * loading/skeleton states for generated content

### **Accessibility considerations for design**

* Meet **WCAG 2.1 AA** contrast in both light and dark mode.  
* Visible focus indicators for keyboard users.  
* Avoid color-only indicators (pair with icons/labels).  
* Support reduced motion (respect OS “Reduce Motion” setting).
...
## **12\) Accessibility (Core Requirements)**
...
## **13\) Analytics & Telemetry (Privacy-aware)**
...
## **14\) Acceptance Criteria Checklist (v1)**

### **Core**

* Users can authenticate (login or new account).  
...
## **15\) Implementation Notes (LLM/Dev-Oriented)**
