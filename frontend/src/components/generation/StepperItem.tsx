...
export function StepperItem({ index, objectiveLabel, progress }: StepperItemProps) {
  const p = progress ?? { planned: false, planTitle: null, written: false, activityCreated: false, activityId: null, error: null };

  const steps = [
    { label: p.planTitle ? `Planned: ${p.planTitle}` : 'Planning lesson', done: p.planned },
  ...
  return (
    <div className="flex gap-3">
      <div className="flex flex-col items-center">
        <StepIcon done={allDone} active={anyStarted && !allDone} />
        ...
        <div className="mt-2 space-y-1">
          {steps.map((step, i) => (
            <div key={i} className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              ) : anyStarted && !steps.slice(0, i).every((s) => s.done) ? (
